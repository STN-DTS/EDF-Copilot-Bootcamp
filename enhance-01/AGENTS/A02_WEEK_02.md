# A02 — Week 02 — Normalize Labs + Rails + Embedded Prompts

## Scope
Normalize Week 02 micro-labs (constraint extraction/journey mapping/AC writing/review). Ensure learners can progress linearly.

## Owned paths (hard boundary)
- `docs/content/week-02/**`
- `bootcamp/steps/week-02/**`
...
1. **Navigation rails**
   - Add `Previous | Home | Next` rail at the end of every:
     - `docs/content/week-02/README.md`
     - `docs/content/week-02/micro-labs/*.md`
...
3. **Step metadata completion**
   - Ensure `bootcamp/steps/week-02/` contains a YAML file per micro-lab in this week.
   - Use the YAML schema from the cleanup spec (id, read, touch, prompts, validate, nav).
   - If the week uses sub-labs (e.g., LAB_2_0 …), use unique IDs (e.g., `week-02-lab-02-0`, etc.).
   - Update `bootcamp/steps/index.yaml` (if owned) or log required additions for A00.

4. **Local link fixes**
   - Fix internal links that break within `docs/content/week-02/**` after your edits.
