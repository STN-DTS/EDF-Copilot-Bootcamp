# A04 — Week 04 — Normalize Labs + Rails + Embedded Prompts

## Scope
Normalize Week 04 micro-labs (OpenAPI endpoint/glossary term/MSW handler/contract validation). Ensure rails and embedded prompts.

## Owned paths (hard boundary)
- `docs/content/week-04/**`
- `bootcamp/steps/week-04/**`
...
1. **Navigation rails**
   - Add `Previous | Home | Next` rail at the end of every:
     ...
     - `docs/content/week-04/micro-labs/*.md`
...
3. **Step metadata completion**
   - Ensure `bootcamp/steps/week-04/` contains a YAML file per micro-lab in this week.
   - Use the YAML schema from the cleanup spec (id, read, touch, prompts, validate, nav).
   - If the week uses sub-labs (e.g., LAB_4_0 …), use unique IDs (e.g., `week-04-lab-04-0`, etc.).
   - Update `bootcamp/steps/index.yaml` (if owned) or log required additions for A00.

4. **Local link fixes**
   - Fix internal links that break within `docs/content/week-04/**` after your edits.
