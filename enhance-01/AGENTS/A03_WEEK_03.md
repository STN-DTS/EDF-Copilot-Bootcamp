# A03 — Week 03 — Normalize Labs + Rails + Embedded Prompts

## Scope
Normalize Week 03 micro-labs (ADR drafting/constitution/spec persona/review). Ensure rails and embedded prompts.

## Owned paths (hard boundary)
- `docs/content/week-03/**`
- `bootcamp/steps/week-03/**`
...
1. **Navigation rails**
   - Add `Previous | Home | Next` rail at the end of every:
     - `docs/content/week-03/README.md`
     - `docs/content/week-03/micro-labs/*.md`
...
3. **Step metadata completion**
   - Ensure `bootcamp/steps/week-03/` contains a YAML file per micro-lab in this week.
   - Use the YAML schema from the cleanup spec (id, read, touch, prompts, validate, nav).
   - If the week uses sub-labs (e.g., LAB_3_0 …), use unique IDs (e.g., `week-03-lab-03-0`, etc.).
   - Update `bootcamp/steps/index.yaml` (if owned) or log required additions for A00.

4. **Local link fixes**
   - Fix internal links that break within `docs/content/week-03/**` after your edits.
